terraform {
  ...
  required_providers {
    openstack = {
      source  = "terraform.local/openstack"
      version = "3.0.0"
    }
  }
}
...
resource "openstack_networking_secgroup_v2" "guseva_group" {
  name        = "guseva-group"
  description = "SSH access for guseva-instance"
}

resource "openstack_networking_secgroup_rule_v2" "guseva_group_ssh" {
  security_group_id = openstack_networking_secgroup_v2.guseva_group.id
  direction         = "ingress"
  ethertype         = "IPv4"
  protocol          = "tcp"
  port_range_min    = 22
...
resource "openstack_networking_port_v2" "server_port" {
  ...
  security_group_ids = [
    ...
    openstack_networking_secgroup_v2.guseva_group.id,
  ]
}

# --- Создаём инстанс, используя порт ---
resource "openstack_compute_instance_v2" "guseva_server" {
  name              = "guseva-instance-01-trf"
  ...
  depends_on = [
    openstack_networking_secgroup_v2.guseva_group,
  ]
}

# --- Выводим IP для доступа и Ansible ---
output "server_ip" {
  description = "IPv4 address of guseva-instance-01"
  value       = openstack_compute_instance_v2.guseva_server.access_ip_v4
}

# --- Ждём, пока SSH сервис станет доступным ---
resource "null_resource" "wait_for_ssh" {
  depends_on = [openstack_compute_instance_v2.guseva_server]

  connection {
    type             = "ssh"
    user             = "ubuntu"
    host             = openstack_compute_instance_v2.guseva_server.access_ip_v4
...
resource "null_resource" "generate_inventory" {
  ...
  triggers = {
    server_ip = openstack_compute_instance_v2.guseva_server.access_ip_v4
  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      echo "${openstack_compute_instance_v2.guseva_server.access_ip_v4}" >> hosts.ini
    EOT
  }
}
